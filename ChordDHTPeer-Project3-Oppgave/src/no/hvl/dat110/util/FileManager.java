...
public class FileManager {
	...
	public void createReplicaFiles() {
		...
		// Task:given a filename, create nreplicas (1 to nreplicas)- idea, append each
		// index to the filename before hash

		// hash the replica using the Hash.hashOf() and store it in an array

		// return the replicas as array of BigInteger

		replicafiles = new BigInteger[Util.numReplicas];
	...
	public int distributeReplicastoPeers() throws RemoteException {
		...
		while (counter < Util.numReplicas) {
			// for each replica, find its successor by performing findSuccessor(replica)
			NodeInterface stub = chordnode.findSuccessor(replicafiles[counter]);
			// call the addKey on the successor and add the replica
			stub.addKey(replicafiles[counter]);
			// call the saveFileContent() on the successor
			stub.saveFileContent(filename, replicafiles[counter], bytesOfFile, counter == index);
			// increment counter
			counter++;
		}
